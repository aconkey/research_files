...
**** DONE On-line processing of grammatical structure using reservoir computing - Hinaut 2012
     ...
     - They provide some validation for their results by considering also a classification task in which the data (inputs and assigned classifications) are just randomized vectors, the idea being that if the network were not in fact picking up on the semantic structure of the sentences, then there would not be an appreciable difference between the sentence task and the randomized data task.
...
**** DONE Real-time parallel processing of grammatical structure in the fronto-striatal system (reservoir computing) - Hinaut 2013
    ...
    I am not sure how strong their results really are. They report a 42% sentence error rate (at least one of the output nodes incorrect) and potentially very close to 50% on leave-one-out cross validation. To me that seems like a high error rate.
...
**** DONE A practical guide to applying echo state networks - Lukosevicius 2012
     ...
     - convergence in backpropagation in RNNs is not guaranteed (bifurcations) and if it converges it can be very slow.

     - "the reservoir acts (i) as a nonlinear expansion and (ii) as a memory of input at the same time... Both aspects (i) and (ii) combined, the reservoir, being an input-driven dynamical system, should provide a rich and relevant enough signal space in x(n), such that the desired target y_t(n) could be obtained by a linear combination from it."

     - "The general wisdom is that the bigger the reservoir, the better the obtainable performance, provided appropriate regularization measures are taken against overfitting."

     - "Alpha can be regarded as the time interval in the continuous world between two consectuive time steps in the discrete realization... When the task requires modeling the time series producting dynamical system on multiple time scales, it might be useful to set different leaking rates to different units (vector) with a possible downside of having more parameters to optimize."

       - This may be relevant for me. Processing dependencies in a sentence or over the course of a larger corpus can happen at different rates. E.g. the dependencies between words or phrases can occur in close proximity (word pair dependencies) or can occur over the longer range of an entire sentence or even over multiple sentences. It may not be necessary to have every unit with a differnt leaking rate, but maybe specify classes of alpha values (say, 10 alpha values over a certain range), and distribute the units more or less uniformly over those classes.

**** TODO Reservoir Computing Trends - Lukosevicius-Jaeger 2012
     [[../papers/lukojaeger2012.pdf][Local PDF]]

**** DONE A tutorial on training recurrent neural networks, covering BPPT, RTRL, EKF and the "echo state network" approach - Jaeger 2002
     CLOSED: [2015-05-03 Sun 20:24]
     [[http://www.pdx.edu/sites/www.pdx.edu.sysc/files/Jaeger_TrainingRNNsTutorial.2005.pdf][Paper PDF]]

     "A randomly constructed RNN, such as our dynamic reservoir, might develop oscillatory or even chaotic activity even in the absence of external excitation. We do not want this to occur: The ESN approach needs a DR which is damped, in the sense that if the network is started from an arbitrary state, the subsequent network states converge to the zero state. This can be achieved by a proper global scaling of W: the smaller the weights of W, the stronger the damping."

     This damping seems essential for my task because the idea is that semantically similar encoded sentences should produce similar dynamics in the reservoir. This is unlikely to happen though if the network dynamics settle near chaos because then even the same input applied at different times might incite dynamics that diverge drastically.

     "We do not collect information from times n=1,...,100 because the network's dynamics is partially determined by the network's arbitrary starting state. By time n=100 we can safely assume that the effects of the arbitrary string state have died out and that the network states are a pure reflection of the teacher forced signal."

     Important to note that the reservoir will need to time to respond appropriately to signals. It seems that I would also have to introduce "cleansing" perionds between input signals so that the network has appropriate time to adjust to the current input and process it appropriately. Maybe consider sending a signal in as pulses (i.e. repeating same signal one after other as an input train) so that the reservoir begins to resonate for that signal and will settle into an oscillatory state that is representative of that input that can then be read out. 

     (Sine-wave teacher forcing example:) "Most networks trained according to the presription given here can be started from almost any arbitrary nonzero starting state and will lock into the desired sinewave. In mathematical terms, the trained network is a dynamical system with a single attractor, and this attractor is the desired oscillation. However, the strong stability observed in this example is a pleasant side-effect of the simplicity of the sinewave generating task. When the tasks become more difficult, the stability of the trained dynamics is indeed a critical issue for ESN training."

     They note that the echo state property of a network is dependent on the training data, i.e. it the weights and connections assigned irrespective of the training will exhibit echo states for some training date and not for others.

     "The diligent choice of the spectral radius alpha of the DR weight matrix is of crucial importance for the eventual success of ESN training. This is because alpha is intimately connected to the intrinsic timescale of the dynamics of the DR state. Small alpha means that one has a fast DR, large alpha (i.e. close to unity) means that one has a slow DR. The intrinsic timescale of the task should match the DR timescale."

     Step-by-step help on pg. 30 for initial construction of ESN

**** TODO Harnessing non-linearity: predicting chaotic systems and saving energy in wireless communications - Jaeger 2004
     [[../papers/jaeger2004.pdf][Local PDF]]

**** TODO Optimization and applications of echo state networks with leaky-integrator neurons - Jaeger 2007
     [[../papers/jaeger2007][LocalPDF]]

**** TODO Phoneme Recognition with Large Hierarchical Reservoirs - Triefenbach 2011
     [[../papers/triefenbach2011.pdf][Local PDF]]
...
**** TODO Bifurcations in the Learning of Recurrent Neural Networks - Doya 1992
     [[../papers/doya1992.pdf][Local PDf]]

**** TODO Real-time Computing Without Stable States: A New Framework for Neural Computation Based on Perturbation - Maass 2002
     [[../papers/maass2002.pdf][Local PDF]]

**** TODO Echo state networks as an alternative to traditional artificial neural networks in rainfall-runoff modelling
     [[http://www.hydrol-earth-syst-sci.net/17/253/2013/hess-17-253-2013.pdf][Paper PDF]]

**** DONE Effects of spectral radius and settling time in the performance of echo state networks - Shishir 2009
     CLOSED: [2015-06-21 Sun 10:56]
     [[../papers/shishir2009.pdf][Local PDF]]
** Networks
